...
faa_name = sys.argv[1] #Put the name of the protein.faa file with the python call
...
for record in SeqIO.parse(ogp, "fasta"):
	name = record.id
	strname = str(record.description)
	xploc = re.search("protein_id=",strname).end()
	xpnum = strname[xploc:xploc+14]
	if search(longesttx, xpnum):
		outfile.write(">")
